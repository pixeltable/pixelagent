...
if TABLE_NAME not in pxt.list_tables():
    ...
    video_index = pxt.create_table(
        TABLE_NAME, {"video": pxt.Video, "uploaded_at": pxt.Timestamp}
    )
    ...
    video_index.add_computed_column(
        transcription=openai.transcriptions(
            audio=video_index.audio_extract, model=WHISPER_MODEL
        )
    )
    ...
    transcription_chunks = pxt.create_view(
        ...
        iterator=StringSplitter.create(
            text=video_index.transcription_text, separators="sentence"
        ),
    )
...
video_index.insert(
    {"video": video, "uploaded_at": datetime.now()} for video in videos[:2]
)
...
result = agent.run(
    agent_name="Video_Search",
    message="Explain what the author meant by what is happiness?",
)
